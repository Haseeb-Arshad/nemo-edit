...
app.use((req, res, next) => {
    const id = req.__reqId || randomUUID().slice(0, 8);
    req.__reqId = id;
...
app.get("/styles", async (req, res) => {
    try {
        ...
        if (categorySlug) {
            ...
            if (cat)
                stylesQuery = stylesQuery.eq("category_id", cat.id);
            else
                stylesQuery = stylesQuery.eq("category_id", "00000000-0000-0000-0000-000000000000");
        }
...
app.get("/prompts", async (req, res) => {
    try {
        ...
        if (styleSlug) {
            ...
            if (style)
                query = query.eq("style_id", style.id);
            else
...
app.post("/generate-image", upload.single("image"), async (req, res) => {
    try {
        ...
        const styleSlug = (req.body.styleSlug || req.body.style || null);
        const promptId = (req.body.promptId || null);
        const promptText = (req.body.promptText || req.body.prompt || undefined);
        ...
        console.log(`[req ${reqId}] /generate-image succeeded`, { taskId: result?.taskId, outputs: result?.outputs?.length || 0 });
        res.json(result);
    }
    catch (e) {
        const reqId = req.__reqId;
...
app.post("/api/v1/edit", upload.fields([{ name: "file", maxCount: 1 }, { name: "mask", maxCount: 1 }]), async (req, res) => {
    try {
        ...
        await updateGenerationTask(task.id, { params: { ...(task.params || {}), originalPath, maskPath } });
        // Kick off generation in background and return immediately
        (async () => {
            try {
                const maskBuf = mask ? mask.buffer : null;
...
app.get("/api/v1/edit/:id", async (req, res) => {
    try {
        ...
        if (task.status === "succeeded") {
            ...
            const first = outputs?.[0];
            if (first)
                result_url = getPublicUrl({ bucket: first.storage_bucket, path: first.storage_path });
        }
        return res.json({ job_id: id, status, result_url });
    }
    catch (e) {
        res.status(500).json({ error: e?.message || "Unknown error" });
    }
