...
export async function uploadBufferToStorage(opts: {
  ...
  if (isPinataEnabled()) {
    ...
    if (config.pinataPreferIpfs) {
      ...
      if (!cid) throw new Error("Pinata pinFileToIPFS did not return a CID");
...
export async function signedUrl(opts: { bucket: string; path: string; expiresIn?: number }) {
...
export function getPublicUrl(opts: { bucket: string; path: string }) {
  if (opts.bucket === "external-url") {
    return opts.path;
  }
  if (isPinataEnabled()) {
...
export async function downloadToBase64(opts: { bucket: string; path: string }) {
  if (opts.bucket === "external-url") {
  ...
  if (error) throw error;
  const arr = Buffer.from(await (data as any).arrayBuffer());
  return arr.toString("base64");
}
...
export async function insertGenerationTask(payload: Partial<GenerationTaskRow>) {
  if (!supabaseAdmin) throw new Error("Supabase not configured");
...
export async function updateGenerationTask(id: string, patch: Partial<GenerationTaskRow>) {
  if (!supabaseAdmin) throw new Error("Supabase not configured");
  const { data, error } = await supabaseAdmin
    .from("generation_tasks")
    .update(patch)
