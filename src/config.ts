...
export const config = {
  ...
  provider: (process.env.PROVIDER || "gemini") as "gemini" | "mock",
...
export function assertEnv() {
  if (!config.geminiApiKey) console.warn("GEMINI_API_KEY is not set; /generate-image will fail");
  if (!config.supabaseUrl) console.warn("SUPABASE_URL is not set; DB/storage features will fail");
  if (!config.supabaseServiceRoleKey) console.warn("SUPABASE_SERVICE_ROLE_KEY is not set; DB/storage features will fail");
  if (!config.pinataJwt) console.warn("PINATA_JWT is not set; image uploads to Pinata will fail");
}
